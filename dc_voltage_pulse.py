...
class DcVoltagePulse(Measurement):
    ...
    def setup(self):
        ...
        # Duration
        s.New(
            "Number of Cycles", int, initial=0,
    ...
    class DataManager():
        ...
        def plot_update(self):
            ...
            cfg = self.registry[dataset_name]
            for line, s in zip(self.plot_lines, cfg.series):
                line.setData(self.data.get(s.x_data_name, []), self.data.get(s.y_data_name, []))
...
#
        def data_reset(self):
            ...
            """
            self.data = {
                # Measurement Data from the references
                "ref. time" : [],
                "ref. current" : [], 
                "ref. resistance" : [],
                "ref. cycle" : [],
    ...
    def setup_figure(self):
        ...
        mset_layout.addWidget(
            self.settings.New_UI(
                include = ("Meas. Range", "NPLC","Continuous",
                           "Number of Cycles",),
    ...
    def run(self):
        ...
        def start_zero_cycle(num_calibration_pts=5): 
            ...
            self.hw.write_voltage(s["Reference DC Voltage"])
        ...
        def step_voltage_routine(pulse_or_reference,cycle_num):
            ...
            if pulse_or_reference == 'pulse':
                self.hw.write_voltage(s['Pulse DC Voltage'])
